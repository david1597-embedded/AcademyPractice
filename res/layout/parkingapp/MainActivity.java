...
public class MainActivity extends AppCompatActivity {
    ...
    private void beginListenForData() {
        ...
        if (inputStream == null) {
            Toast.makeText(this, "Cannot get bluetooth input stream", Toast.LENGTH_SHORT).show();
            return;
        }
        workerThread = new Thread(() -> {
            while (!Thread.currentThread().isInterrupted() && !stopWorker.get()) {
                try {
                    ...
                    stopWorker.set(true);
                    Log.e(TAG, "I/O Error occurred while receiving data", e);
                    handler.post(() -> Toast.makeText(MainActivity.this, "Disconnected bluetooth", Toast.LENGTH_SHORT).show());
                } catch (InterruptedException e) {
                    stopWorker.set(true);
                    Log.e(TAG, "Terminated data receiving thread", e);
                }
            }
        });
        workerThread.start();
